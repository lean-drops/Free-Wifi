...
openai.api_key = 'your_openai_api_key'  # Ersetze durch deinen OpenAI API-Key
...
# OpenAI API Anfrage für erweiterte Datenverarbeitung
...
def packet_callback(packet):
    if packet.haslayer(IP):
        ...
        # Erweiterte Datenverarbeitung durch OpenAI
        prompt = f"Analysiere die folgenden Daten:\nQuell-IP: {source_ip}\nZiel-IP: {dest_ip}\nProtokoll: {protocol}\nPayload: {payload}"
        analysis = openai_analysis(prompt)
...
if __name__ == "__main__":
    ...
    if not user_interaction("Möchten Sie die MAC-Adresse ändern?"):
        exit()
    ...
    if not user_interaction("Möchten Sie Ihr eigenes WLAN testen?"):
        exit()

    # Test am eigenen WLAN
    test_own_wlan(interface)

    if not user_interaction("Möchten Sie Netzwerke scannen?"):
        exit()
